...
func (m *Repository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range rooms {
		// Get block map from session. Loop through it
		// if we have an entry in the map that does not exist in posted data
		// and if the restriction id > 0, then it is a block that needs to be removed
		curMap := m.App.Session.Get(r.Context(), fmt.Sprintf("block_map_%d", x.ID)).(map[string]int)
		for name, value := range curMap {
			// ok will be false if value not in map
			if val, ok := curMap[name]; ok {
				// only pay attention to values > 0, and are not in the form post (i.e. unchecked)
